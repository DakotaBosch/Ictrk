...
import { getRouteCoordinates } from './mapboxDirections';
 ...
 * @param {Array} routeCoords - Route coordinates for determining orientation
...
/**
 * Fallback function for drawing routes when Mapbox fails
 ...
 */
function drawFallbackRoute(L, map, shipment) {
  console.log("Using fallback route drawing");
  
  // Make sure we have valid data
  if (!map || !shipment || !shipment.checkpoints || shipment.checkpoints.length < 2) {
    console.warn('Invalid data for fallback route');
    return null;
  }

  try {
    // Get coordinates from checkpoints
    const checkpointCoords = shipment.checkpoints.map(cp => cp.coords).filter(coord => 
      coord && Array.isArray(coord) && coord.length === 2 && 
      !isNaN(coord[0]) && !isNaN(coord[1])
    );
    
    if (checkpointCoords.length < 2) {
      console.warn('Not enough valid coordinates for fallback route');
      return null;
    }
    
    // Create a polyline with the coordinates
    const routeLine = L.polyline(checkpointCoords, {
      color: shipment.mode === 'Air' ? '#3b82f6' :
             shipment.mode === 'Land' ? '#8b5cf6' :
             shipment.mode === 'Ocean' ? '#0891b2' : '#6b7280',
      weight: 3,
      opacity: 0.7,
      dashArray: '5, 5' // Dashed to indicate it's a fallback route
    }).addTo(map);
    
    // Create bounds and fit the map to the route
    try {
      const bounds = L.latLngBounds(checkpointCoords);
      if (bounds.isValid()) {
      ...
      console.warn('Error fitting bounds in fallback route:', e);
    }
    
    // Calculate vehicle position based on progress
    let vehiclePosition = null;
    try {
      vehiclePosition = getPositionAlongRoute(checkpointCoords, shipment.progress);
    } catch (e) {
      console.warn('Error calculating vehicle position:', e);
    }
    ...
    if (vehiclePosition) {
      ...
      try {
        vehicleMarker = L.marker(vehiclePosition, {
          icon: createTransportIcon(L, shipment.transportType, shipment.progress, checkpointCoords)
        }).addTo(map);
        ...
        console.warn('Error creating vehicle marker:', e);
      }
    }
...
/**
 * Draws a route on the map based on transport type with road snapping when appropriate
 * @param {Object} L - Leaflet library instance
 * @param {Object} map - Leaflet map instance
 * @param {Object} shipment - Shipment data object
 * @returns {Promise<Object>} Promise resolving to object containing route line and vehicle marker
 */
export const drawShipmentRoute = async (L, map, shipment) => {
  console.log("Drawing shipment route", {
    mode: shipment?.mode,
    transportType: shipment?.transportType
  });

  if (!map || !shipment || !shipment.checkpoints || shipment.checkpoints.length < 2) {
    console.warn('Missing required data for route drawing');
    return null;
  }
  ...
  try {
    ...
    } else if (shipment.mode === 'Land') {
      // For land routes, determine the appropriate transport mode for Mapbox
      let transportMode = 'driving'; // Default
      
      if (shipment.transportType === 'truck') {
        transportMode = 'driving-traffic';
      } else if (shipment.transportType === 'walking') {
        transportMode = 'walking';
      } else if (shipment.transportType === 'cycling') {
        transportMode = 'cycling';
      }
      
      console.log('Using road snapping for land route with mode:', transportMode);
      
      try {
        // Get road-snapped coordinates from Mapbox
        const mbxCoords = await getRouteCoordinates(
          shipment.checkpoints,
          transportMode
        );
        
        if (Array.isArray(mbxCoords) && mbxCoords.length > 0) {
          // Mapbox returns [longitude, latitude] but Leaflet uses [latitude, longitude]
          // Check the format of the first coordinate to determine if conversion is needed
          if (mbxCoords[0].length === 2 && typeof mbxCoords[0][0] === 'number') {
            // If it looks like a Mapbox response [lng, lat], convert it
            if (Math.abs(mbxCoords[0][0]) > 90) { // Longitude is typically > 90, latitude is not
              console.log('Converting Mapbox coordinates to Leaflet format');
              routeCoords = mbxCoords.map(coord => [coord[1], coord[0]]);
            } else {
              // Already in Leaflet format
              routeCoords = mbxCoords;
            }
          } else {
            // Unable to determine format, use as-is
            routeCoords = mbxCoords;
          }
        } else {
          throw new Error('Invalid or empty coordinates from Mapbox');
        }
      } catch (error) {
        console.warn('Road snapping failed, using straight lines:', error);
        routeCoords = shipment.checkpoints.map(cp => cp.coords);
      }
    } else {
      // For ocean or other modes, use straight lines
      routeCoords = shipment.checkpoints.map(cp => cp.coords);
    }

    // Validate route coordinates
    routeCoords = routeCoords.filter(coord => 
      coord && Array.isArray(coord) && coord.length === 2 &&
      !isNaN(coord[0]) && !isNaN(coord[1])
    );
    
    if (routeCoords.length < 2) {
      throw new Error('Not enough valid route coordinates');
    }
    ...
    // Only call fallback if we haven't already tried to draw a route
    if (!layers.routeLine) {
      return drawFallbackRoute(L, map, shipment);
    }
